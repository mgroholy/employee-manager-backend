...
public class EmployeeManagerApplication {
    ...
    public CommandLineRunner init() {
        return args -> {
            ...
            Position purchasingLeader = Position.builder().name("Purchasing Leader").build();
            positionRepository.saveAll(Arrays.asList(developer, recruiter, purchasingLeader));
            ...
            Employee jack = Employee.builder().name("Jack Smith").email("jack@smith.com").department(SALES).phoneNumber("36309876543").dateOfBirth(LocalDate.parse("1960-05-25")).clearanceLevel(ClearanceLevel.SUPERVISOR).position(purchasingLeader).status(Status.ACTIVE).dateOfHire(LocalDate.of(2005, 6, 10)).build();
